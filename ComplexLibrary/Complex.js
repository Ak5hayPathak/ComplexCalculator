export class Complex {
  ...
  printComplex(precision = 3) {
    ...
    if (this.isPureReal()) {
      ...
      console.log(
        this.real.toFixed(precision) +
          (this.imag > 0 ? " + " : " - ") +
          (Math.abs(this.imag) === 1
            ? "i"
            : Math.abs(this.imag).toFixed(precision) + "i")
      );
    }
  }
  ...
  static random(min = 0, max = 1) {
    ...
    return new Complex(randomValue(), randomValue());
  }
  ...
  toString(precision = 3) {
    ...
    const imagPart =
      Math.abs(this.imag) === 1 ? "i" : format(Math.abs(this.imag)) + "i";
    const sign = this.imag >= 0 ? "+" : "-";
    return realPart + sign + imagPart;
  }

  static get ZERO() {
    return Object.freeze(new Complex(0, 0));
  }
  static get ONE() {
    return Object.freeze(new Complex(1, 0));
  }
  static get NEG_ONE() {
    return Object.freeze(new Complex(-1, 0));
  }
  static get IOTA() {
    return Object.freeze(new Complex(0, 1));
  }
  static get NEG_IOTA() {
    return Object.freeze(new Complex(0, -1));
  }
  ...
  static get OMEGA() {
    return Object.freeze(new Complex(-0.5, Math.sqrt(3) / 2.0));
  }
  static get OMEGA_SQR() {
    return Object.freeze(new Complex(-0.5, -Math.sqrt(3) / 2.0));
  }

  static get LOG_OF_NEG_UNITY() {
    return Object.freeze(new Complex(0, Math.PI));
  }
  static get LOG_OF_IOTA() {
    return Object.freeze(new Complex(0, Math.PI / 2.0));
  }
  static get LOG_OF_OMEGA() {
    return Object.freeze(new Complex(0, (2 * Math.PI) / 3.0));
  }
}
