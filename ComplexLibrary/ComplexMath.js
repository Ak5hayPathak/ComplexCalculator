...
export class ComplexMath {
  ...
  static multiply(num1, num2, isImaginary = false) {
    ...
    if (typeof num1 === "number" && typeof num2 === "number") {
      return isImaginary
        ? new Complex(0, num1 * num2)
        : new Complex(num1 * num2, 0);
    }
  ...
  static divide(num1, num2, isImaginary = false) {
    let div = function (numerator, denominator) {
      if (
        !(numerator instanceof Complex) ||
        !(denominator instanceof Complex)
      ) {
        throw new Error(
          "Both numerator and denominator must be Complex numbers"
        );
      }
      ...
      return new Complex(
        numeratorModified.real / denom,
        numeratorModified.imag / denom
      );
    };
  ...
  if (num1 instanceof Complex) {
    return new Complex(Math.floor(num1.real), Math.floor(num1.imag));
  } else {
    return Math.floor(num1); // works for regular numbers
  }
}

static ceil(num1) {
  if (num1 instanceof Complex) {
    return new Complex(Math.ceil(num1.real), Math.ceil(num1.imag));
  } else {
    return Math.ceil(num1); // works for regular numbers
  }
}
