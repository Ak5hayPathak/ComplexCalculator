...
export class Button {
  ...
  static numVal(numValue, display1, display2) {
    ...
    if (
      numValue === "0" &&
      (/[eE][+-]?$/.test(display2.value) || /[eE][+-]?$/.test(display1.value))
    )
      return;
  ...
  static iota(display1, display2) {
    ...
    if (
      /[eE][+-]?$/.test(display2.value) ||
      /[eE][+-]?$/.test(display1.value)
    ) {
      return;
    }
  ...
  static Brackets(display1, display2) {
    ...
    if (
      /[eE][+-]?$/.test(display2.value) ||
      (/[eE][+-]?$/.test(display1.value) && display2.value === "")
    ) {
      return;
    }
    ...
    if (display2.value === "") {
      if (display1.value !== "") {
        ...
        if (matchedFunc) {
          ...
          if (!(matchedFunc === "log" || matchedFunc === "root"))
            this.bracketCount--;
        } else {
  ...
  static updateBracketDisplay(openBracketBtn) {
    this.openBracketBtn.innerText =
      this.bracketCount === 0 ? "" : `${this.bracketCount}`;
  }
  ...
  static toggleInverseTrigono() {
    ...
    toggleButton.addEventListener("mouseenter", () => {
      toggleButton.style.backgroundColor = operatorStateTrigono.isInverse
        ? "#cf3000"
        : "#303030"; // Add hover effect
    });

    toggleButton.addEventListener("mouseleave", () => {
      toggleButton.style.backgroundColor = operatorStateTrigono.isInverse
        ? "#FF3D00"
        : "#3a3a3a"; // Revert to default
    });
  ...
  static toggleHyperTrigono() {
    ...
    toggleButton.addEventListener("mouseenter", () => {
      toggleButton.style.backgroundColor = operatorStateTrigono.isHyper
        ? "#cf3000"
        : "#303030"; // Add hover effect
    });
  ...
  static insertConstant(constant, display1, display2) {
    if (
      ...
      return;
    if (
      display2.value.endsWith(".") ||
      display1.value.endsWith(".") ||
      display1.value.endsWith("E")
    )
      return;
    if (
      /[eE][+-]?$/.test(display2.value) ||
    ...
    if (display2.value === "") {
      if (display1.value === "") {
        ...
        if (endsWithOperator(display1.value)) {
          ...
          display1.value +=
            (needsMultiply ? "*" : "") + funcString + (addBracket ? "(" : "");
          if (addBracket) needsBracketUpdate();
        }
      }
    } else {
  ...
  static createHistoryContainer(display1, display2) {
    ...
    // Create expressionValue input
    const expressionValue = document.createElement("input");
    expressionValue.className = "expressionValue";
    expressionValue.autocomplete = "off";
    expressionValue.value = "";
    expressionValue.readOnly = true;

    // Pre-fill history only if display1 and display2 have values
    if (display1.value !== "" && display2.value !== "") {
      expression.value = display1.value;
      expressionValue.value = display2.value;
    }

    // Append inputs to container
    container.appendChild(expression);
    container.appendChild(expressionValue);

    // Add click event
    container.addEventListener("click", () => {
      if (display1 && display2) {
        display1.value = expression.value;
        display2.value = expressionValue.value;
      }
    });
    ...
    if (display1.value !== "" || display2.value !== "") {
      ...
      try {
        ...
        const result = output.toString
          ? output.toString(precision)
          : output.toFixed(precision);
        display2.value = result;
      } catch (e) {
        display2.value = "Invalid";
      }
      this.createHistoryContainer(display1, display2);
